...
WakeLab is a standalone training and workbench application for
openWakeWord. It does not train a wake word by itself. Instead, it
helps you:
...
`src/orac_wake_lab/themes/set_theme.txt` as `<theme-name>:<mode>`, for
example `Phoenix:Dark`.
...
- **Use constructed parts**: toggle between the simple pronunciation
  field and the split positive-generation fields.
- **Training pronunciation phrase**: simple single-phrase mode used when
  the toggle is off.
- **Training phrase parts**: optional `|`-separated positive-generation
  fragments such as `Hey | Nova`. Wake Lab generates each part separately
  and inserts real silence between parts so TTS word-boundary collapse is
  not baked into the training positives. The toggle must be on for this
  field group to be used.
...
- **Export Model**: copies the selected model into the selected target
  directory.
- **Output textbox**: shows the exported model path, export details, and
  any ONNX sidecar copied with it.
